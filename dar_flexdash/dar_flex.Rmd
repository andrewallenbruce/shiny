...
output$downloadData <- downloadHandler(
    filename = function() {
        paste(datadl, "dar-data-", Sys.Date(), ".csv", sep = ".")
    },
    content = function(file) {
        write.csv(datadl, file, na = "NA", dec = ".")
    }
)
```
...
-   The plot on the right shows your `total Gross Charges` (x-axis) and `Ending AR balance` (y-axis) as a point, labelled with your current `Days in AR`. 

-   You want the point to be **above** and to the **left** of the diagonal line (representing the `Ideal Ratio` for the `Number of Days in the Period`) for `Days in AR` to be under the `Days in AR` target.
...
### DAR Key Indicators: Gross Charges ($x$), Ending AR ($y$), Number of Days in Period ($m$) {data-height="100%"}
        ...
        ggrepel::geom_label_repel(aes(
            ...
            scale_y_continuous(
                labels = scales::dollar_format(
                    prefix="$"), 
                limits = c(0, earb() * 2.25)) +
            scale_x_continuous(
                labels = scales::dollar_format(
                    prefix="$"), 
                limits = c(0, input$gct * 2.25))+
        tidyquant::theme_tq(base_size = 12) +
