...
sidebar <- dashboardSidebar(
        sidebarMenu(
            menuItem("DAR Calculator", tabName = "darcalc", icon = icon("fas fa-chart-bar"), startExpanded =TRUE,
                     numericInput(
                         "dart",
                         "Days in AR Target:",
                         min = 1,
...
body <- dashboardBody(
            fluidRow(
                ...
                infoBoxOutput("darBox"),
                infoBoxOutput("artBox"),
                box(title = "DAR Indicator: Above & Left of the Dotted line, DAR is Failing", width = 6, solidHeader = TRUE, status = "warning",
...
server <- function(input, output) {
    ...
    output$darBox <-renderInfoBox({
        infoBox(
            "Days in AR", paste0(dar(), " Days"),
            icon = icon("fas fa-chart-bar")
        )
    })
    ...
    output$artBox <-renderInfoBox({
        infoBox("Ending AR Target", paste0("$", earbtarg()),
                icon = icon("fas fa-chart-bar")
        )
    })
